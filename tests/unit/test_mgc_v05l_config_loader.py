...
def test_runtime_exclusive_probationary_package_replaces_prior_lane_payloads(tmp_path: Path) -> None:
    base_path = tmp_path / "base.yaml"
    overlay_path = tmp_path / "overlay.yaml"
    base_path.write_text(
        "\n".join(
            [
                ...
                "vol_len: 20",
                "show_debug_labels: false",
                f"probationary_paper_lanes_json: '{json.dumps([{'lane_id': 'legacy_lane'}])}'",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    overlay_path.write_text(
        "\n".join(
            [
                ...
                f"probationary_paper_lanes_json: '{json.dumps([{'lane_id': 'exclusive_lane'}])}'",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert [lane.get("lane_id") for lane in settings.probationary_paper_lane_specs] == ["exclusive_lane"]

...
def test_dead_paper_runtime_with_clean_broker_state_is_restart_eligible() -> None:
    ...
    assert ("bash", "scripts/run_headless_supervised_paper_service.sh", "--no-start-dashboard", "--wait-timeout-seconds", "120", "--post-start-pid-wait-timeout-seconds", "45") in plan["actions"][0]["commands"]
...
def test_apply_restarts_paper_runtime_through_supervised_launcher(tmp_path: Path) -> None:
    ...
    assert calls[1][:3] == ("bash", "scripts/run_headless_supervised_paper_service.sh", "--no-start-dashboard")

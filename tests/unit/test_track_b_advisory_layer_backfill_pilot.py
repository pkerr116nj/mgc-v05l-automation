...
def test_apply_writes_episode_centric_advisory_rows_and_manifests(tmp_path: Path) -> None:
    ...
    assert manifest["raw_candidate_counts"] == {"GC": 3}
    assert manifest["skipped_by_dedupe"] == {"GC": 1}
    assert validation["expected_rows_per_layer"] == 8

    import pyarrow.parquet as pq

    lifecycle_path = Path(result["output_paths"]["lifecycle_awareness_parquet"])
...
def test_expected_episode_mismatch_stops_on_compatibility_uncertainty(tmp_path: Path) -> None:
    ...
    assert "EXPECTED_EPISODE_COUNT_MISMATCH" in str(excinfo.value)
...
def test_apply_refuses_to_overwrite_existing_outputs(tmp_path: Path) -> None:
    ...
    assert "refusing_to_overwrite_existing_outputs" in str(excinfo.value)
...
def _write_symbol_inputs(candidate_root: Path, base_root: Path, symbol: str) -> None:
    shard = "2024Q1"
    candidate_path = (
        candidate_root
        / symbol
    ...
    hot_cache_path = base_root / symbol / "2024" / "Q1" / "bars.parquet"
    hot_manifest_path = hot_cache_path.parent / "partition_manifest.json"
    start = datetime(2024, 1, 1, 0, 0, tzinfo=UTC)
    candidates = [
        _candidate(symbol, start),
        _candidate(symbol, start + timedelta(minutes=20), suffix="deduped-away"),
        _candidate(symbol, start + timedelta(minutes=65), suffix="second"),
    ]
...
def _candidate(symbol: str, decision_ts: datetime, *, suffix: str = "first") -> dict[str, object]:
    return {

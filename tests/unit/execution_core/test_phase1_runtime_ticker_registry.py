...
def test_registry_classifies_full_size_micro_and_rates() -> None:
    ...
    assert {symbol for symbol, item in registry.items() if item.contract_type == "full_size"} == {
        "GC",
        "NQ",
        "ES",
        "PL",
    ...
    assert {symbol for symbol, item in registry.items() if item.contract_type == "micro"} == {
        "MGC",
        "MNQ",
        "MES",
        "MBT",

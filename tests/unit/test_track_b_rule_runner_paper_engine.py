...
def test_crypto_active_evidence_specs_are_diagnostics_only_not_active() -> None:
    for symbol in ("BTC", "MBT", "ETH", "MET", "SOL", "MSL"):
        assert f"PAPER_ACTIVE_EVIDENCE_{symbol}_US_PARTICIPATION_LONG_V1" not in PAPER_ACTIVE_EVIDENCE_SPECS
        assert f"PAPER_ACTIVE_EVIDENCE_{symbol}_LONDON_LATE_PARTICIPATION_SHORT_V1" not in PAPER_ACTIVE_EVIDENCE_SPECS
...
def test_rule_runner_native_fallback_maps_mnq_us_derivative_bear_turn_to_intent() -> None:
    fallback = _native_runtime_fallback_spec({"rule_mode": "MNQ_US_DERIVATIVE_BEAR_TURN_V1"}, _LaneSpec())
    assert fallback is not None
    assert fallback.short_family is ShortEntryFamily.DERIVATIVE_BEAR

    payload = _empty_signal_packet_payload("MNQ|5m|2026-05-28T14:35:00Z")
    payload.update(
        {
            "short_entry_raw": True,
            "recent_short_setup": True,
    ...
    promoted_packet = _promoted_signal_packet_from_native(
        "MNQ|5m|2026-05-28T14:35:00Z",
        SignalPacket(**payload),
        fallback,
    )
    bar = _bar(datetime(2026, 5, 28, 10, 35, tzinfo=ZoneInfo("America/New_York")), open_="21020", close="20980")
    ...
    engine._latest_track_b_rule_report = {
        ...
        "entry_source": "MNQ_US_DERIVATIVE_BEAR_TURN_V1",
    ...
    assert promoted_packet.short_entry is True
    assert promoted_packet.short_entry_source == "MNQ_US_DERIVATIVE_BEAR_TURN_V1"
    assert intent is not None
    assert intent.intent_type is OrderIntentType.SELL_TO_OPEN
    assert intent.reason_code == "MNQ_US_DERIVATIVE_BEAR_TURN_V1"
...
def test_crypto_paper_active_evidence_sources_are_not_active() -> None:
    ny = ZoneInfo("America/New_York")
    history = [
    ...
    assert long_decision["accepted"] is False
...
def test_london_late_active_evidence_short_accepts_with_canonical_0530_reference(tmp_path) -> None:
    ny = ZoneInfo("America/New_York")
    anchor_path = tmp_path / "outputs/track_b_execution_core/session_anchors/MNQ/2026-05-28/LONDON_LATE_0530_REFERENCE.json"
    anchor_path.parent.mkdir(parents=True, exist_ok=True)
    anchor_path.write_text(
        json.dumps(
            {
    ...
    engine = object.__new__(TrackBRuleRunnerPaperStrategyEngine)
    engine._bar_history = [
        _bar(datetime(2026, 5, 28, 5, 45, tzinfo=ny), open_="20990", close="20980"),
        _bar(datetime(2026, 5, 28, 5, 50, tzinfo=ny), open_="20980", close="20970"),
    ]
    engine._settings = SimpleNamespace(symbol="MNQ", trade_size=1)
    engine._track_b_repo_root = tmp_path
...
def test_london_late_active_evidence_short_blocks_when_0530_reference_missing(tmp_path) -> None:
    ny = ZoneInfo("America/New_York")
    engine = object.__new__(TrackBRuleRunnerPaperStrategyEngine)
    engine._bar_history = [_bar(datetime(2026, 5, 28, 5, 45, tzinfo=ny), open_="20990", close="20980")]
    engine._settings = SimpleNamespace(symbol="MNQ", trade_size=1)
    engine._track_b_repo_root = tmp_path

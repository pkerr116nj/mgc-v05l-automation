...
BAR_COUNT="$(sqlite3 "${DB_PATH}" "select count(*) from bars;")"
if [[ "${BAR_COUNT}" -le 0 ]]; then
  echo "No bars found in ${DB_PATH}. Run Schwab historical persistence first." >&2
  exit 1
fi
...
run = repositories.experiment_runs.create(
    ExperimentRunRecord(
        name="real-data-ema-run-current",
        description="EMA research run on current persisted Schwab bars",
...
print({"run_id": run_id, "bars_loaded": len(bars), "stage": "ema_research_complete"})
PY
...
if [[ ! -f "${REPORT_PATH}" ]]; then
  echo "Expected report artifact was not created: ${REPORT_PATH}" >&2
  exit 1
fi

if [[ ! -f "${VIZ_PATH}" ]]; then
  echo "Expected visualization artifact was not created: ${VIZ_PATH}" >&2
  exit 1
fi

"""CLI for the read-only Track B PAPER managed-order registry authority."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the read-only Track B PAPER managed-order registry artifact.")
    parser.add_argument("--repo-root", type=Path, default=REPO_ROOT)
    parser.add_argument("--output-path", type=Path, default=DEFAULT_MANAGED_ORDER_REGISTRY_ARTIFACT)
    parser.add_argument("--event-log-path", type=Path, default=DEFAULT_MANAGED_ORDER_EVENTS)
    parser.add_argument("--dashboard-projection-path", type=Path, default=DEFAULT_DASHBOARD_MANAGED_ORDER_PROJECTION)
...
def _run_once(args: argparse.Namespace) -> None:
    ...
    payload = build_track_b_managed_order_registry(config=config, now=datetime.now().astimezone())
    if args.no_write:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    output_path, events = write_track_b_managed_order_registry(config=config, payload=payload)
    print(
        json.dumps(
            {

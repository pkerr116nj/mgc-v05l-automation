"""Replay-first runner for Asia Drift v1 Phase 1 research artifacts."""
...
from ..research.asia_drift import RECOVERY_CONFIRMED, run_asia_drift_phase1


REPO_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_SOURCE_DB = REPO_ROOT / "mgc_v05l.replay.sqlite3"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "outputs" / "reports" / "asia_drift_v1_phase1"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="asia-drift-v1-phase1")
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Optional output directory. Defaults to a timestamped folder under outputs/reports/asia_drift_v1_phase1.",
    )
    return parser


def run_asia_drift_v1_phase1(
    *,
    source_db: Path,
    ...
    return {
        ...
        "summary_json_path": str(run.artifacts.summary_json_path),
        "summary_markdown_path": str(run.artifacts.summary_markdown_path),
        "feature_rows_path": str(run.artifacts.feature_rows_path),
        "state_rows_path": str(run.artifacts.state_rows_path),
        "session_summaries_path": str(run.artifacts.session_summaries_path),
        "candidate_manifest_path": str(run.artifacts.candidate_manifest_path),
        "storage_manifest_path": str(run.artifacts.storage_manifest_path),
        "feature_row_count": len(run.feature_rows),
        "state_row_count": len(run.state_rows),
        "session_count": len(run.session_summaries),
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_asia_drift_v1_phase1(

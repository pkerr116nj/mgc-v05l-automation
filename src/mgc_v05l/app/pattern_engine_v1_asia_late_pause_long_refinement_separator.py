"""Robustness separator analysis inside the tested ASIA_LATE pause long branch."""
...
BASE_SEPARATOR_PHASE_INDEX_LEFT = 1
BASE_SEPARATOR_PHASE_INDEX_RIGHT = 2
BASE_SEPARATOR_FIELD = "expansion_state"
BASE_SEPARATOR_LEFT_VALUE = "COMPRESSED"
BASE_SEPARATOR_RIGHT_VALUE = "NORMAL"
...
def build_and_write_asia_late_pause_long_refinement_separator(*, detail_csv_path: Path) -> dict[str, str]:
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_asia_late_pause_long_refinement_lane_summary.json")
    candidate_csv_path = prefix.with_suffix(".pattern_engine_v1_asia_late_pause_long_refinement_candidates.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_asia_late_pause_long_refinement_summary.json")
...
def _load_lane_rows(detail_csv_path: Path) -> list[SeparatorLaneRow]:
    ...
    with detail_csv_path.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if primitive_steps[BASE_SEPARATOR_PHASE_INDEX_LEFT][BASE_SEPARATOR_FIELD] != BASE_SEPARATOR_LEFT_VALUE:
                continue
            if primitive_steps[BASE_SEPARATOR_PHASE_INDEX_RIGHT][BASE_SEPARATOR_FIELD] != BASE_SEPARATOR_RIGHT_VALUE:
                continue
            rows.append(
                SeparatorLaneRow(
...
def _build_lane_summary(rows: list[SeparatorLaneRow]) -> dict[str, Any]:
    ...
    return {
        ...
        "base_separator": {
            ...
            "primitive_field": BASE_SEPARATOR_FIELD,
            "primitive_value": f"{BASE_SEPARATOR_LEFT_VALUE}->{BASE_SEPARATOR_RIGHT_VALUE}",
        },
...
def _baseline_metrics(rows: list[SeparatorLaneRow]) -> dict[str, Decimal]:
    ...
    return {
        ...
        "recent_avg_move_10bar": _avg(row.move_10bar for row in rows if _slice_name(row.anchor_dt) == "recent"),
        "recent_favorable_move10_rate": _ratio(
            sum(1 for row in rows if _slice_name(row.anchor_dt) == "recent" and row.move_10bar > 0),
            sum(1 for row in rows if _slice_name(row.anchor_dt) == "recent"),
        ),
    }
...
def _candidate_row(
    *,
    ...
    recent_drag = recent_avg_move_10bar < 0
...
def _build_summary_payload(*, lane_summary: dict[str, Any], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decay_drivers = [
        row
        for row in repeatable_candidates
        if Decimal(row["recent_avg_move_10bar"]) < 0 or row["recent_drag_flag"]
    ]
    decay_drivers.sort(
        key=lambda row: (
    ...
    return {
        ...
        "largest_recent_decay_driver": decay_drivers[0] if decay_drivers else None,
...
def main() -> None:
    ...
    outputs = build_and_write_asia_late_pause_long_refinement_separator(detail_csv_path=args.detail_csv_path)

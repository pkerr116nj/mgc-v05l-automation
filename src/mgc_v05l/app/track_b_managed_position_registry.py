"""CLI for the read-only Track B PAPER managed-position registry authority."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the read-only Track B PAPER managed-position registry artifact.")
    parser.add_argument("--repo-root", type=Path, default=REPO_ROOT)
    parser.add_argument("--output-path", type=Path, default=DEFAULT_MANAGED_POSITION_REGISTRY_ARTIFACT)
    parser.add_argument("--event-log-path", type=Path, default=DEFAULT_MANAGED_POSITION_EVENTS)
    parser.add_argument("--dashboard-projection-path", type=Path, default=DEFAULT_DASHBOARD_MANAGED_POSITION_PROJECTION)
    parser.add_argument("--no-dashboard-projection", action="store_true")
    parser.add_argument("--once", action="store_true", help="Run one registry pass and exit.")
...
def _run_once(args: argparse.Namespace) -> None:
    ...
    payload = build_track_b_managed_position_registry(config=config, now=datetime.now().astimezone())
    if args.no_write:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    output_path, events = write_track_b_managed_position_registry(config=config, payload=payload)
    print(
        json.dumps(
            {

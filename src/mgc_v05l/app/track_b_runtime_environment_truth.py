"""CLI for read-only Track B PAPER runtime environment truth."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the read-only Track B PAPER runtime environment truth artifact.")
    parser.add_argument("--repo-root", type=Path, default=REPO_ROOT)
    parser.add_argument("--output-path", type=Path, default=DEFAULT_RUNTIME_ENVIRONMENT_TRUTH_ARTIFACT)
    parser.add_argument("--event-log-path", type=Path, default=DEFAULT_RUNTIME_ENVIRONMENT_EVENTS)
    parser.add_argument("--dashboard-projection-path", type=Path, default=DEFAULT_DASHBOARD_RUNTIME_ENVIRONMENT_PROJECTION)
    ...
    parser.add_argument("--no-write", action="store_true", help="Print payload without writing artifacts.")
    return parser
...
def _run_once(args: argparse.Namespace) -> None:
    config = TrackBRuntimeEnvironmentTruthConfig(
        ...
        dashboard_projection_path=None if args.no_dashboard_projection else Path(args.dashboard_projection_path),
    )
    payload = build_track_b_runtime_environment_truth(config=config, now=datetime.now().astimezone())
    if args.no_write:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    output_path, events = write_track_b_runtime_environment_truth(config=config, payload=payload)
    print(
        json.dumps(
            {

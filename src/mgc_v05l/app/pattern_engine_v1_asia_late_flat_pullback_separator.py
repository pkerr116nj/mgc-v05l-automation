"""Separator analysis inside the tightened ASIA_LATE flat-pullback branch."""
...
def build_and_write_asia_late_flat_pullback_separator(*, detail_csv_path: Path) -> dict[str, str]:
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_asia_late_flat_pullback_lane_summary.json")
    candidate_csv_path = prefix.with_suffix(".pattern_engine_v1_asia_late_flat_pullback_candidates.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_asia_late_flat_pullback_summary.json")
    ...
    with detail_csv_path.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if primitive_steps[1]["expansion_state"] != "COMPRESSED":
                continue
            if primitive_steps[2]["expansion_state"] != "NORMAL":
                continue
            if primitive_steps[1]["curvature_state"] != "CURVATURE_FLAT":
                continue
            rows.append(
                SeparatorLaneRow(
...
def _build_lane_summary(rows: list[SeparatorLaneRow]) -> dict[str, Any]:
    ...
    return {
        ...
        "base_separators": [
            {"phase_path": "pullback->resumption", "primitive_field": "expansion_state", "primitive_value": "COMPRESSED->NORMAL"},
            {"phase_path": "pullback", "primitive_field": "curvature_state", "primitive_value": "CURVATURE_FLAT"},
        ],
...
def _build_candidate_rows(rows: list[SeparatorLaneRow]) -> list[dict[str, Any]]:
    ...
    for bucket_name in ("ASIA_LATE_OPEN", "ASIA_LATE_MID", "ASIA_LATE_CLOSE"):
        bucket = [row for row in rows if _asia_late_time_bucket(row.anchor_dt) == bucket_name]
...
def _candidate_row(
    *,
    ...
    return {
        ...
        "recent_estimated_value_mfe20": str(sum((row.mfe_20bar for row in recent_rows), Decimal("0"))),
        "slice_breakdown": json.dumps(_slice_metrics(rows), sort_keys=True, default=str),
    }
...
def _separator_quality(
    *,
    ...
    if (
        ...
        and mfe_mae_lift > Decimal("1.10")
        and avg_move_10bar > 0
        and recent_avg_move_10bar >= Decimal("0")
    ):
        return "candidate"
...
def _build_summary_payload(*, lane_summary: dict[str, Any], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    recent_decay = [row for row in repeatable_candidates if Decimal(row["recent_avg_move_10bar"]) < 0]
    recent_decay.sort(key=lambda row: (Decimal(row["recent_avg_move_10bar"]), -row["match_count"]))
    resilient = [row for row in promotable if int(row["recent_match_count"]) > 0]
    resilient.sort(
        key=lambda row: (
            Decimal(row["usefulness_score"]),
            Decimal(row["recent_avg_move_10bar"]),
            row["match_count"],
        ),
        reverse=True,
    )
    return {
        ...
        "best_recent_resilient_discriminator": resilient[0] if resilient else None,
        "largest_recent_decay_driver": recent_decay[0] if recent_decay else None,
...
def _slice_metrics(rows: list[SeparatorLaneRow]) -> dict[str, dict[str, str | int]]:
    result: dict[str, dict[str, str | int]] = {}
    for slice_name in ("early", "middle", "recent"):
        bucket = [row for row in rows if _slice_name(row.anchor_dt) == slice_name]
        avg_move = _avg(row.move_10bar for row in bucket)
...
def _asia_late_time_bucket(anchor_dt: datetime) -> str:
    local_time = anchor_dt.timetz().replace(tzinfo=None)
    if time(20, 30) <= local_time < time(21, 30):
        return "ASIA_LATE_OPEN"
    if time(21, 30) <= local_time < time(22, 15):
        return "ASIA_LATE_MID"
    return "ASIA_LATE_CLOSE"
...
def main() -> None:
    ...
    outputs = build_and_write_asia_late_flat_pullback_separator(detail_csv_path=args.detail_csv_path)

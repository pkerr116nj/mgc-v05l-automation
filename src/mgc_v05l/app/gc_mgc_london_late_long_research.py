"""Research study for a GC/MGC London-late continuation long model."""
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "gc_mgc_london_late_long"
...
LONDON_LATE_START = time(5, 30)
LONDON_LATE_END = time(8, 20)
DEFAULT_TICK_SIZE = 0.1


@dataclass(frozen=True)
class LondonLateLongSpec:
    variant_id: str
    description: str
    ...
    max_pre_context_to_setup_range_ratio: float = 1.0
    min_setup_abs_efficiency: float = 0.45
    min_setup_close_location: float = 0.65
    min_setup_vwap_displacement: float = 0.08
    min_setup_green_share: float = 0.67
    min_setup_volume_ratio: float = 0.95
    require_last_setup_high_break: bool = True
    gate_mode: str = "strict"
    min_setup_score: int = 0
    meta_filter_mode: str = "none"
    max_meta_pre_context_ratio: float | None = None
    min_meta_close_location: float | None = None
    min_meta_vwap_displacement: float | None = None
    min_meta_green_share: float | None = None
    min_meta_score: int = 0
    exit_mode: str = "segment_close"
...
@dataclass(frozen=True)
class LondonLateSessionContext:
    trade_date: date
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gc-mgc-london-late-long-research")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_gc_mgc_london_late_long_research(
...
def run_gc_mgc_london_late_long_research(
    *,
    ...
    for symbol in resolved_symbols:
        ...
        session_contexts_by_timeframe = {
            timeframe: _build_london_late_session_contexts(
        ...
        variant_sessions: dict[str, list[LondonLateTrade]] = {}
    ...
    report = {
        ...
        "study_id": "gc_mgc_london_late_long",
        "study_status": "research_first_pass",
        "definition": {
            "description": (
                "London-late continuation long model: use the first three completed 5m bars of the London-late segment "
                "as the setup, require compression-to-expansion plus strong close quality, then buy the first confirmed "
                "breakout close above the setup high and manage with either a segment-close or EMA-structure exit."
            ),
            "segment_window": {
                "segment_id": "LONDON_LATE",
                "start_time": LONDON_LATE_START.isoformat(timespec="minutes"),
                "end_time": LONDON_LATE_END.isoformat(timespec="minutes"),
            },
            "variants": [asdict(spec) for spec in variants],
            "notes": [
                "This is a dedicated gold-native study for the top-ranked London-late long cell from the segment regime scan.",
                "All decisions are made from canonical 1m bars with 5m resampling for the setup and management surface.",
    ...
    json_path = resolved_output_dir / "gc_mgc_london_late_long_research.json"
    markdown_path = resolved_output_dir / "gc_mgc_london_late_long_research.md"
    ...
    return {
        "mode": "gc_mgc_london_late_long_research",
...
def build_variant_specs(*, selected_ids: list[str] | tuple[str, ...] | None = None) -> tuple[LondonLateLongSpec, ...]:
    variants = (
        LondonLateLongSpec(
            variant_id="london_late_long_v1",
            description="Balanced compression-breakout continuation with segment-close exit.",
            max_pre_context_to_setup_range_ratio=1.05,
            min_setup_abs_efficiency=0.40,
            min_setup_close_location=0.62,
            min_setup_vwap_displacement=0.05,
            min_setup_green_share=0.67,
            min_setup_volume_ratio=0.90,
            exit_mode="segment_close",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v2",
            description="Stricter quality gate with EMA-plus-structure exit.",
            max_pre_context_to_setup_range_ratio=0.90,
            min_setup_abs_efficiency=0.50,
            min_setup_close_location=0.70,
            min_setup_vwap_displacement=0.12,
            min_setup_green_share=0.67,
            min_setup_volume_ratio=1.00,
            exit_mode="ema_structure",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v2b_5m_soft",
            description="Softer 5m score-based continuation gate around the v2 shape.",
            max_pre_context_to_setup_range_ratio=1.20,
            min_setup_abs_efficiency=0.32,
            min_setup_close_location=0.56,
            min_setup_vwap_displacement=0.02,
            min_setup_green_share=0.67,
            min_setup_volume_ratio=0.80,
            require_last_setup_high_break=False,
            gate_mode="score",
            min_setup_score=5,
            exit_mode="ema_structure",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v2c_5m_softer",
            description="Even softer 5m score gate to test whether sample size expands without losing all edge.",
            max_pre_context_to_setup_range_ratio=1.35,
            min_setup_abs_efficiency=0.25,
            min_setup_close_location=0.52,
            min_setup_vwap_displacement=0.0,
            min_setup_green_share=0.34,
            min_setup_volume_ratio=0.70,
            require_last_setup_high_break=False,
            gate_mode="score",
            min_setup_score=4,
            exit_mode="ema_structure",
        ...
        LondonLateLongSpec(
            variant_id="london_late_long_v3",
            description="Tight compression and stronger pressure requirements with EMA-plus-structure exit.",
            max_pre_context_to_setup_range_ratio=0.75,
            min_setup_abs_efficiency=0.58,
            min_setup_close_location=0.78,
            min_setup_vwap_displacement=0.15,
            min_setup_green_share=1.00,
            min_setup_volume_ratio=1.05,
            exit_mode="ema_structure",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v4_10m",
            description="10m smoother continuation model with a two-bar setup and EMA-plus-structure exit.",
            decision_timeframe="10m",
            setup_bar_count=2,
            entry_lookahead_bars=3,
            max_pre_context_to_setup_range_ratio=0.95,
            min_setup_abs_efficiency=0.45,
            min_setup_close_location=0.68,
            min_setup_vwap_displacement=0.10,
            min_setup_green_share=1.00,
            min_setup_volume_ratio=0.95,
            exit_mode="ema_structure",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v5_3m_soft",
            description="3m score-based continuation model for finer London-late timing.",
            ...
            min_setup_vwap_displacement=0.03,
        ...
        LondonLateLongSpec(
            variant_id="london_late_long_v6_7m_soft",
            description="7m score-based continuation model for a smoother but still early London-late read.",
            decision_timeframe="7m",
            setup_bar_count=2,
            entry_lookahead_bars=4,
            max_pre_context_to_setup_range_ratio=1.10,
            min_setup_abs_efficiency=0.35,
            min_setup_close_location=0.60,
            min_setup_vwap_displacement=0.05,
            min_setup_green_share=1.00,
            min_setup_volume_ratio=0.85,
            require_last_setup_high_break=False,
            gate_mode="score",
            min_setup_score=5,
            exit_mode="ema_structure",
        ...
        LondonLateLongSpec(
            variant_id="london_late_long_v7_3m_meta",
            description="3m soft pool plus meta-filter for tighter pre-context and stronger close quality.",
            decision_timeframe="3m",
            setup_bar_count=4,
            entry_lookahead_bars=6,
            max_pre_context_to_setup_range_ratio=1.20,
            ...
            max_meta_pre_context_ratio=1.60,
            min_meta_close_location=0.78,
            min_meta_vwap_displacement=0.18,
            min_meta_green_share=0.50,
            min_meta_score=3,
            exit_mode="ema_structure",
        ),
        LondonLateLongSpec(
            variant_id="london_late_long_v8_5m_meta",
            description="5m soft pool plus meta-filter for tighter pre-context and stronger VWAP/close quality.",
            max_pre_context_to_setup_range_ratio=1.35,
            min_setup_abs_efficiency=0.25,
            min_setup_close_location=0.52,
            min_setup_vwap_displacement=0.0,
            ...
            min_setup_score=4,
            meta_filter_mode="score",
            max_meta_pre_context_ratio=1.45,
            min_meta_close_location=0.78,
            min_meta_vwap_displacement=0.16,
            min_meta_green_share=0.67,
            min_meta_score=3,
            exit_mode="ema_structure",
        ),
    )
...
def evaluate_symbol_sessions(
    *,
    symbol: str,
    spec: LondonLateLongSpec,
    session_contexts: list[LondonLateSessionContext],
) -> list[LondonLateTrade]:
    rows: list[LondonLateTrade] = []
...
def _build_london_late_session_contexts(
    *,
...
) -> list[LondonLateSessionContext]:
    grouped_5m: dict[date, list[ResearchBar]] = defaultdict(list)
    for bar in decision_bars:
        if label_gold_segment(bar.end_ts) != "LONDON_LATE":
            continue
        ...
        if end_day is not None and trade_day > end_day:
            continue
        grouped_5m[trade_day].append(bar)
    ...
    contexts: list[LondonLateSessionContext] = []
    for trade_day in sorted(grouped_5m):
        start_dt = datetime.combine(trade_day, LONDON_LATE_START, tzinfo=NEW_YORK)
        ...
        contexts.append(
            LondonLateSessionContext(
                trade_date=trade_day,
                pre_context=[bar for _, bar in local_1m[left:right]],
                segment_bars=grouped_5m[trade_day],
            )
        )
    return contexts


def _evaluate_london_late_session(
    *,
    symbol: str,
    trade_day: date,
    pre_context: list[ResearchBar],
    ...
    if len(segment_bars) < spec.setup_bar_count + 2:
        return _blank_trade(symbol=symbol, trade_day=trade_day, variant_id=spec.variant_id, notes=("insufficient_segment_bars",))
    if len(pre_context) < max(5, spec.pre_context_minutes // 2):
        return _blank_trade(symbol=symbol, trade_day=trade_day, variant_id=spec.variant_id, notes=("insufficient_pre_context",))
    ...
    last_setup_high_break = setup_bars[-1].high > max(bar.high for bar in setup_bars[:-1])
    ...
    quality_checks = {
        ...
        "last_setup_high_break_ok": last_setup_high_break if spec.require_last_setup_high_break else True,
    }
    ...
    if qualified and spec.meta_filter_mode == "score":
        meta_checks = {
            ...
            "meta_close_location_ok": (
                spec.min_meta_close_location is not None and setup_close_location >= spec.min_meta_close_location
            ),
            "meta_vwap_displacement_ok": (
                spec.min_meta_vwap_displacement is not None and setup_vwap_displacement >= spec.min_meta_vwap_displacement
            ),
            "meta_green_share_ok": spec.min_meta_green_share is not None and setup_green_share >= spec.min_meta_green_share,
        }
    ...
    trigger_price = _round_price(setup_high + spec.tick_size)
    stop_price = _round_price(setup_low - spec.tick_size)
    breakout_index = _find_breakout_close_index(segment_bars=segment_bars, trigger_price=trigger_price, spec=spec)
    if breakout_index is None or breakout_index + 1 >= len(segment_bars):
        return LondonLateTrade(
            symbol=symbol,
            trade_date=trade_day.isoformat(),
            variant_id=spec.variant_id,
            ...
            net_pnl_dollars=None,
            execution_cost_points=None,
            risk_points=_round_price(trigger_price - stop_price),
            gross_r_multiple=None,
            net_r_multiple=None,
            mae_points=None,
            mfe_points=None,
    ...
    pnl_points = _round_price(exit_price - entry_price)
    ...
    mfe_points = _round_price(max(0.0, max(bar.high for bar in trade_window) - entry_price))
    mae_points = _round_price(max(0.0, entry_price - min(bar.low for bar in trade_window)))
...
def _blank_trade(*, symbol: str, trade_day: date, variant_id: str, notes: tuple[str, ...]) -> LondonLateTrade:
    return LondonLateTrade(
        symbol=symbol,
        trade_date=trade_day.isoformat(),
...
def _find_breakout_close_index(*, segment_bars: list[ResearchBar], trigger_price: float, spec: LondonLateLongSpec) -> int | None:
    ...
    *,
    ...
    for index in range(entry_index, len(segment_bars)):
        ...
        if spec.exit_mode == "ema_structure" and index > entry_index:
            ...
            if ema_value is not None and bar.close < ema_value and bar.close < segment_bars[index - 1].low:
...
def _summarize_trades(rows: list[LondonLateTrade]) -> dict[str, Any]:
...
def _cross_symbol_ranking(symbol_reports: dict[str, Any], *, variants: tuple[LondonLateLongSpec, ...]) -> list[dict[str, Any]]:
...
def _contract_economics(symbol: str, *, spec: LondonLateLongSpec) -> ContractEconomics:
...
def _execution_cost_points(*, spec: LondonLateLongSpec, economics: ContractEconomics) -> float:
...
def _load_start_timestamp(start_day: date | None) -> datetime | None:
    if start_day is None:
        return None
    return datetime.combine(start_day, time(5, 0), tzinfo=NEW_YORK)
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    for row in report["cross_symbol_ranking"]:
        lines.append(
            f"- `{row['variant_id']}`: min avg net pnl `{row['min_average_net_pnl_points']}`, min PF `{row['min_net_profit_factor']}`, min trades `{row['min_entered_trade_count']}`"
        )

"""Executable paper-runtime adapter for the promoted GC/MGC forced-session baseline."""
...
LONDON_EARLY_LONG_SOURCE_LEGACY = "londonEarlyLongV2"
NY_EARLY_SHORT_SOURCE = "nyEarlyShortV2"
NY_LATE_SHORT_SOURCE = "nyLateShortV2"


@dataclass(frozen=True)
class ForcedSessionRuntimeDefinition:
    source_id: str
    lane_id: str
    segment_id: str
    side: str
...
FORCED_SESSION_RUNTIME_DEFINITIONS: tuple[ForcedSessionRuntimeDefinition, ...] = (
    ForcedSessionRuntimeDefinition(
        source_id=ASIA_EARLY_SHORT_SOURCE,
        lane_id="ASIA_EARLY_SHORT",
        segment_id="ASIA_EARLY",
        side="SHORT",
    ),
    ForcedSessionRuntimeDefinition(
        source_id=ASIA_EARLY_LONG_SOURCE,
    ...
    ForcedSessionRuntimeDefinition(
        source_id=LONDON_EARLY_LONG_SOURCE,
    ...
    ForcedSessionRuntimeDefinition(
        source_id=LONDON_EARLY_LONG_SOURCE_LEGACY,
        lane_id="LONDON_EARLY_LONG",
        segment_id="LONDON_EARLY",
        side="LONG",
    ),
    ForcedSessionRuntimeDefinition(
        source_id=NY_EARLY_SHORT_SOURCE,
        lane_id="US_EARLY_SHORT",
        segment_id="US_EARLY",
        side="SHORT",
    ),
    ForcedSessionRuntimeDefinition(
        source_id=NY_LATE_SHORT_SOURCE,
...
FORCED_SESSION_RUNTIME_BY_SOURCE = {
    definition.source_id: definition for definition in FORCED_SESSION_RUNTIME_DEFINITIONS
}


class GcMgcForcedSessionStrategyEngine(StrategyEngine):
    """StrategyEngine-backed executable adapter for the forced-session gold baseline."""
    ...
    def _evaluate_signals(self, feature_packet: FeaturePacket, feature_history: list[FeaturePacket]) -> SignalPacket:
        ...
        if str(current_bar.symbol or "").upper() not in {"GC", "MGC"}:
            return SignalPacket(**payload)
        if label_gold_segment(current_bar.end_ts) != definition.segment_id:
            return SignalPacket(**payload)
    ...
    def _resolve_long_entry_family(self, signal_packet: SignalPacket) -> LongEntryFamily:
        if signal_packet.long_entry_source in {
            ...
            LONDON_EARLY_LONG_SOURCE_LEGACY,
        }:
    ...
    def _resolve_short_entry_family(self, signal_packet: SignalPacket) -> ShortEntryFamily:
        if signal_packet.short_entry_source in {ASIA_EARLY_SHORT_SOURCE, NY_EARLY_SHORT_SOURCE, NY_LATE_SHORT_SOURCE}:
    ...
    def _forced_session_exit_intent(
        ...
        *,
        ...
        if not should_exit and _is_last_bar_of_segment(
            bar=current,
            segment_id=definition.segment_id,
            timeframe=self._primary_context_timeframe,
        ):
            should_exit = True
    ...
    def _segment_bars_for_timestamp(self, timestamp) -> list[Bar]:
        ...
        if definition is None:
            return []
        trade_day = trade_date_for_timestamp(timestamp)
        return [
            candidate
            for candidate in self._bar_history
            if trade_date_for_timestamp(candidate.end_ts) == trade_day
            and label_gold_segment(candidate.end_ts) == definition.segment_id
        ]


def _resolve_runtime_definition(lane_spec: Any) -> ForcedSessionRuntimeDefinition | None:
    ...
    if source is None:
        return None
    return FORCED_SESSION_RUNTIME_BY_SOURCE.get(str(source))
...
def _is_last_bar_of_segment(*, bar: Bar, segment_id: str, timeframe: str) -> bool:
    ...
    return label_gold_segment(next_end) != segment_id

"""Secondary separator analysis for the promoted ASIA_EARLY pause short branch."""
...
TARGET_FAMILY = "pause_rebound_resume_short"
TARGET_DIRECTION = "SHORT"
TARGET_SESSION = "ASIA_EARLY"
BASE_SEPARATOR_FIELD = "curvature_state"
BASE_SEPARATOR_VALUE = "CURVATURE_FLAT"
PHASE_NAMES = ("setup", "rebound", "resumption", "confirmation")
PRIMITIVE_FIELDS = (
    "slope_state",
    "curvature_state",
    "expansion_state",
...
def build_and_write_asia_early_pause_short_refinement_separator(
    *, detail_csv_path: Path
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_asia_early_pause_short_refinement_lane_summary.json")
    candidate_csv_path = prefix.with_suffix(".pattern_engine_v1_asia_early_pause_short_refinement_candidates.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_asia_early_pause_short_refinement_summary.json")
...
def _load_lane_rows(detail_csv_path: Path) -> list[SeparatorLaneRow]:
    ...
    with detail_csv_path.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if primitive_steps[0][BASE_SEPARATOR_FIELD] != BASE_SEPARATOR_VALUE:
                continue
            rows.append(
                SeparatorLaneRow(
...
def _build_lane_summary(rows: list[SeparatorLaneRow]) -> dict[str, Any]:
    ...
    return {
        ...
        "base_separator": {"phase": "setup", "primitive_field": BASE_SEPARATOR_FIELD, "primitive_value": BASE_SEPARATOR_VALUE},
...
def main(argv: list[str] | None = None) -> int:
    ...
    outputs = build_and_write_asia_early_pause_short_refinement_separator(detail_csv_path=args.detail_csv)

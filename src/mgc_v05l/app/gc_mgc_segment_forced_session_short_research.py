"""Forced one-trade-per-session research for GC/MGC gold segment short playbooks."""
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "gc_mgc_segment_forced_session_short"
NEW_YORK = ZoneInfo("America/New_York")
VALID_SEGMENTS = ("SESSION_OPEN", "ASIA_EARLY", "ASIA_LATE", "LONDON_EARLY", "LONDON_LATE", "US_EARLY", "US_MIDDAY", "US_LATE")


@dataclass(frozen=True)
class ForcedSegmentShortSpec:
    variant_id: str
    description: str
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gc-mgc-segment-forced-session-short-research")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_gc_mgc_segment_forced_session_short_research(
...
def run_gc_mgc_segment_forced_session_short_research(
    *,
    segment_id: str,
    ...
    for symbol in resolved_symbols:
        ...
        variant_sessions: dict[str, list[ForcedSessionShortTrade]] = {}
    ...
    report = {
        ...
        "study_id": "gc_mgc_segment_forced_session_short",
        "study_status": "frequency_first_pass",
        "definition": {
            "segment_id": normalized_segment_id,
            "description": "Forced-session short playbooks: exactly one short attempt per segment with rule-specific preferred trigger and timed fallback.",
    ...
    json_path = resolved_output_dir / "gc_mgc_segment_forced_session_short_research.json"
    markdown_path = resolved_output_dir / "gc_mgc_segment_forced_session_short_research.md"
    ...
    return {
        "mode": "gc_mgc_segment_forced_session_short_research",
        "artifact_paths": {"json": str(json_path), "markdown": str(markdown_path)},
        "cross_symbol_ranking": report["cross_symbol_ranking"],
    }


def build_variant_specs(*, selected_ids: list[str] | tuple[str, ...] | None = None) -> tuple[ForcedSegmentShortSpec, ...]:
    variants = (
        ForcedSegmentShortSpec(
            variant_id="segment_forced_short_v1_breakdown_or_bar6",
            description="Prefer break below the setup low; otherwise short bar 6 open.",
        ...
        ForcedSegmentShortSpec(
            variant_id="segment_forced_short_v3_bar5_always",
            description="Always short bar 5 open using the first four bars for risk framing.",
            fallback_entry_bar=5,
        ),
        ForcedSegmentShortSpec(
            variant_id="segment_forced_short_v4_breakdown_or_bar7",
            description="Prefer break below the setup low; otherwise short bar 7 open.",
            fallback_entry_bar=7,
        ),
        ForcedSegmentShortSpec(
            variant_id="segment_forced_short_v5_contextual_breakdown",
            description="Use segment-aware fallback timing to avoid weak blind short entries.",
            fallback_entry_bar=7,
        ),
    )
    if not selected_ids:
...
) -> ForcedSessionShortTrade:
    if len(segment_bars) <= spec.fallback_entry_bar:
        return ForcedSessionShortTrade(
            symbol=symbol,
...
def _select_entry_index(
    *,
    ...
    if spec.variant_id in {"segment_forced_short_v1_breakdown_or_bar6", "segment_forced_short_v4_breakdown_or_bar7"}:
    ...
    if spec.variant_id == "segment_forced_short_v2_reclaim_fail_or_bar7":
    ...
    if spec.variant_id == "segment_forced_short_v5_contextual_breakdown":
        trigger = setup_low - spec.tick_size
        for index in range(spec.setup_bar_count, min(len(segment_bars), spec.fallback_entry_bar + 1)):
            if segment_bars[index].close <= trigger:
                return index, "preferred_breakdown"
        if segment_id == "US_MIDDAY":
            if setup_vwap_displacement <= 0.15 and setup_return >= 1.0:
                return min(6, len(segment_bars) - 1), "context_bar7_positive_setup"
            if setup_close_location <= 0.35 and setup_range >= 4.0:
                return min(4, len(segment_bars) - 1), "context_bar5_weak_reclaim"
...
def _summarize_trades(rows: list[ForcedSessionShortTrade]) -> dict[str, Any]:
...
def _cross_symbol_ranking(symbol_reports: dict[str, Any], variants: tuple[ForcedSegmentShortSpec, ...]) -> list[dict[str, Any]]:
    ranked = []
    for spec in variants:
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines = [
        f"# GC/MGC {report['definition']['segment_id']} Forced Session Short Research",
        "",

"""Separator analysis for the US_LATE pause_pullback_resume_long lane."""
...
def build_and_write_us_late_pause_long_separator(*, detail_csv_path: Path) -> dict[str, str]:
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_us_late_pause_long_lane_summary.json")
    candidate_csv_path = prefix.with_suffix(".pattern_engine_v1_us_late_pause_long_separator_candidates.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_us_late_pause_long_separator_summary.json")
...
def _build_summary_payload(*, lane_summary: dict[str, Any], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "top_repeatable_candidates": repeatable_candidates[:10],
    }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Separator analysis for the US_LATE pause_pullback_resume_long lane.")
    parser.add_argument("detail_csv")
    args = parser.parse_args()
    outputs = build_and_write_us_late_pause_long_separator(detail_csv_path=Path(args.detail_csv))

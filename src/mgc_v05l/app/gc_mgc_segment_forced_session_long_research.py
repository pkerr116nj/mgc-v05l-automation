"""Forced one-trade-per-session research for GC/MGC gold segment long playbooks."""
...
from .gc_mgc_london_late_long_research import (
    _contract_economics,
    _ema,
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "gc_mgc_segment_forced_session_long"
NEW_YORK = ZoneInfo("America/New_York")
VALID_SEGMENTS = ("ASIA_EARLY", "LONDON_EARLY", "US_MIDDAY", "US_LATE")


@dataclass(frozen=True)
class ForcedSegmentLongSpec:
    variant_id: str
    description: str
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gc-mgc-segment-forced-session-long-research")
    parser.add_argument("--segment-id", default="ASIA_EARLY", help="Gold session segment to evaluate.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_gc_mgc_segment_forced_session_long_research(
...
def run_gc_mgc_segment_forced_session_long_research(
    *,
    segment_id: str,
    ...
    normalized_segment_id = str(segment_id).strip().upper()
    if normalized_segment_id not in VALID_SEGMENTS:
        raise ValueError(f"Unsupported segment_id: {segment_id}")
    ...
    resolved_output_dir = Path(output_dir or (DEFAULT_OUTPUT_DIR / normalized_segment_id.lower())).resolve()
    ...
    for symbol in resolved_symbols:
        ...
        session_contexts_by_timeframe = {
            timeframe: _build_segment_session_contexts(
                decision_bars=bars,
        ...
        variant_sessions: dict[str, list[ForcedSessionLongTrade]] = {}
    ...
    report = {
        ...
        "study_id": "gc_mgc_segment_forced_session_long",
        "study_status": "frequency_first_pass",
        "definition": {
            "segment_id": normalized_segment_id,
            "description": "Forced-session long playbooks: exactly one long attempt per segment with rule-specific preferred trigger and timed fallback.",
    ...
    json_path = resolved_output_dir / "gc_mgc_segment_forced_session_long_research.json"
    markdown_path = resolved_output_dir / "gc_mgc_segment_forced_session_long_research.md"
    ...
    return {
        "mode": "gc_mgc_segment_forced_session_long_research",
        "artifact_paths": {"json": str(json_path), "markdown": str(markdown_path)},
        "cross_symbol_ranking": report["cross_symbol_ranking"],
    }


def build_variant_specs(*, selected_ids: list[str] | tuple[str, ...] | None = None) -> tuple[ForcedSegmentLongSpec, ...]:
    variants = (
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v1_breakout_or_bar6",
            description="Prefer break above the setup high; otherwise buy bar 6 open.",
            fallback_entry_bar=6,
        ),
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v2_dip_reclaim_or_bar7",
            description="Prefer a dip-reclaim after an early pullback; otherwise buy bar 7 open.",
            fallback_entry_bar=7,
        ),
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v3_bar5_always",
            description="Always buy bar 5 open using the first four bars for risk framing.",
            fallback_entry_bar=5,
        ),
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v4_breakout_or_bar7",
            description="Prefer break above the setup high; otherwise buy bar 7 open.",
            fallback_entry_bar=7,
        ),
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v5_dip_reclaim_or_bar8",
            description="Prefer a dip-reclaim after an early pullback; otherwise buy bar 8 open.",
            fallback_entry_bar=8,
        ),
        ForcedSegmentLongSpec(
            variant_id="segment_forced_long_v6_contextual_fallback",
            description="Use segment-aware fallback timing to avoid the weakest blind entry bars.",
            fallback_entry_bar=8,
        ),
    )
    if not selected_ids:
...
def _build_segment_session_contexts(
    *,
    ...
    for bar in decision_bars:
        local_dt = bar.end_ts.astimezone(NEW_YORK)
        if label_gold_segment(local_dt) != segment_id:
            continue
...
def _evaluate_forced_session(
    *,
...
) -> ForcedSessionLongTrade:
    if len(segment_bars) <= spec.fallback_entry_bar:
        return ForcedSessionLongTrade(
            symbol=symbol,
    ...
    entry_index, entry_reason = _select_entry_index(
...
def _select_entry_index(
    *,
    ...
    if spec.variant_id in {"segment_forced_long_v1_breakout_or_bar6", "segment_forced_long_v4_breakout_or_bar7"}:
        trigger = setup_high + spec.tick_size
        for index in range(spec.setup_bar_count, min(len(segment_bars), spec.fallback_entry_bar + 1)):
            if segment_bars[index].close >= trigger:
                return index, "preferred_breakout"
        fallback_label = f"fallback_bar{spec.fallback_entry_bar}"
        return min(spec.fallback_entry_bar - 1, len(segment_bars) - 1), fallback_label
    if spec.variant_id in {"segment_forced_long_v2_dip_reclaim_or_bar7", "segment_forced_long_v5_dip_reclaim_or_bar8"}:
        midpoint = setup_low + ((setup_high - setup_low) * 0.5)
        for index in range(spec.setup_bar_count, min(len(segment_bars), spec.fallback_entry_bar + 1)):
            prev = segment_bars[index - 1]
            cur = segment_bars[index]
            if prev.low <= midpoint and cur.close > prev.high:
                return index, "preferred_dip_reclaim"
        fallback_label = f"fallback_bar{spec.fallback_entry_bar}"
        return min(spec.fallback_entry_bar - 1, len(segment_bars) - 1), fallback_label
    if spec.variant_id == "segment_forced_long_v6_contextual_fallback":
        if segment_id == "ASIA_EARLY":
            midpoint = setup_low + ((setup_high - setup_low) * 0.5)
            for index in range(spec.setup_bar_count, min(len(segment_bars), spec.fallback_entry_bar + 1)):
                prev = segment_bars[index - 1]
                cur = segment_bars[index]
                if prev.low <= midpoint and cur.close > prev.high:
                    return index, "preferred_dip_reclaim"
            if setup_close_location <= 0.72 and setup_vwap_displacement <= 0.15:
                return min(7, len(segment_bars) - 1), "context_bar8_weak_setup"
            if setup_range >= 3.0:
                return min(5, len(segment_bars) - 1), "context_bar6_wide_setup"
            return min(6, len(segment_bars) - 1), "context_bar7_default"
        if segment_id == "LONDON_EARLY":
            trigger = setup_high + spec.tick_size
            for index in range(spec.setup_bar_count, min(len(segment_bars), 8)):
                if segment_bars[index].close >= trigger:
                    return index, "preferred_breakout"
            if setup_close_location >= 0.55 and setup_vwap_displacement >= -0.05 and setup_return >= 0.0:
                return min(6, len(segment_bars) - 1), "context_bar7_trend_setup"
            return min(5, len(segment_bars) - 1), "context_bar6_default"
...
def _summarize_trades(rows: list[ForcedSessionLongTrade]) -> dict[str, Any]:
...
def _cross_symbol_ranking(symbol_reports: dict[str, Any], variants: tuple[ForcedSegmentLongSpec, ...]) -> list[dict[str, Any]]:
    ranked = []
    for spec in variants:
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines = [
        f"# GC/MGC {report['definition']['segment_id']} Forced Session Long Research",
        "",
...
def _load_start_timestamp(start_day: date | None) -> datetime | None:
    if start_day is None:
        return None
    return datetime.combine(start_day - timedelta(days=1), time(17, 0), tzinfo=NEW_YORK)

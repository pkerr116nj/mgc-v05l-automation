"""Focused Pattern Engine v1 lane selection for breakout_retest_hold."""
...
FAMILY_NAME = "breakout_retest_hold"
EXCLUDED_SESSION_PHASES = {"UNCLASSIFIED"}


@dataclass(frozen=True)
class BreakoutMatchRow:
    direction: str
    session_phase: str
...
def build_and_write_pattern_engine_v1_breakout_lane_mining(*, detail_csv_path: Path) -> dict[str, str]:
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_breakout_lane_summary.csv")
    cluster_summary_path = prefix.with_suffix(".pattern_engine_v1_breakout_lane_clusters.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_breakout_lane_summary.json")
    ...
    return {
        "pattern_engine_v1_breakout_lane_summary_path": str(lane_summary_path),
        "pattern_engine_v1_breakout_lane_clusters_path": str(cluster_summary_path),
        "pattern_engine_v1_breakout_lane_summary_json_path": str(summary_json_path),
    }


def _load_rows(detail_csv_path: Path) -> list[BreakoutMatchRow]:
    rows: list[BreakoutMatchRow] = []
...
def _build_lane_rows(rows: list[BreakoutMatchRow]) -> list[dict[str, Any]]:
    grouped: dict[tuple[str, str], list[BreakoutMatchRow]] = defaultdict(list)
    for row in rows:
...
def _build_cluster_rows(rows: list[BreakoutMatchRow]) -> list[dict[str, Any]]:
    grouped: dict[tuple[str, str, str], list[BreakoutMatchRow]] = defaultdict(list)
...
def main() -> None:
    ...
    outputs = build_and_write_pattern_engine_v1_breakout_lane_mining(detail_csv_path=args.detail_csv_path)

...
OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "ibkr_mes_es_lane_submit_port"
PORTING_OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "ibkr_strategy_porting"
SCOPE_SUPPORT_DIR = REPO_ROOT / "outputs" / "reports" / "ibkr_mes_es_scope_support"
GOVERNANCE_OUTPUT_DIR = REPO_ROOT / "outputs" / "reports" / "ibkr_strategy_governance"
REPORT_MD = "ibkr_mes_es_lane_submit_port_report.md"
REPORT_JSON = "ibkr_mes_es_lane_submit_port_report.json"
AUDIT_JSONL = "ibkr_mes_es_lane_submit_port_audit.jsonl"
QUOTE_DIAGNOSTIC_JSON = SCOPE_SUPPORT_DIR / "ibkr_mes_es_quote_diagnostic_report.json"
EXPOSURE_JSON = SCOPE_SUPPORT_DIR / "paper_mes_es_exposure_state.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Process MES/ES lanes into live submit-capable IBKR paper status, lane by lane.")
    ...
    inventory_rows = {
        ...
        if str(row.get("instrument") or "").upper() in {"MES", "ES"}
    }
    intent_rows = {
        str(row.get("strategy_id") or ""): dict(row)
    ...
    exposure = _load_json(EXPOSURE_JSON)

    lane_results: list[dict[str, Any]] = []
    first_order_event: dict[str, Any] | None = None
    overall_classification = "IBKR_MES_ES_SCOPE_READY"
    quote_classification = str(quote_diag.get("classification") or "IBKR_MES_ES_QUOTES_UNAVAILABLE_UNKNOWN")
    ...
    for strategy_id in _ordered_strategy_ids(inventory_rows):
        ...
        quote_gate = quote_classification in {"IBKR_MES_ES_QUOTES_READY", "IBKR_MES_ES_QUOTES_DELAYED_ONLY"}
        exposure_gate = (
            float(exposure.get("broker_net_mes") or 0.0) == 0.0
            and float(exposure.get("strategy_attributed_mes") or 0.0) == 0.0
            and float(exposure.get("broker_minus_ledger_difference") or 0.0) == 0.0
            and float(exposure.get("orphan_exposure") or 0.0) == 0.0
        )
        bridge_gate = (
        ...
        if action in {"NO_ACTION", "HOLD"} and bridge_gate:
            ...
            lane_config = IbkrLaneSubmitPortConfig(
                repo_root=REPO_ROOT,
            ...
            lane_artifacts = run_ibkr_lane_submit_port(config=lane_config)
            write_ibkr_lane_submit_port_artifacts(config=lane_config, artifacts=lane_artifacts)
        ...
        audit_events.append(
            {
                "event_type": "mes_es_lane_evaluated",
    ...
    report = {
        ...
        "scope_classification": "IBKR_MES_ES_SCOPE_READY",
        ...
        "mes_es_exposure_state": exposure,
        "lane_count": len(inventory_rows),
        "processed_lane_count": len(lane_results),
        "lane_results": lane_results,
        "first_order_event": first_order_event,
        "detail": (
            "Processed MES/ES lanes lane-by-lane and all currently emitted NO_ACTION, so no order was submitted."
            if first_order_event is None
            else "Stopped after the first actionable MES/ES broker-path event."
        ),
    }
...
def _quote_source_label_for_lane(*, strategy_id: str, quote_diag: dict[str, Any]) -> str | None:
    symbol = "MES" if strategy_id.startswith("mes_") else "ES"
    row = dict((quote_diag.get("comparison") or {}).get(symbol.lower()) or {})
...
def _blocked_detail(
    *,
    ...
    if not exposure_gate:
        reasons.append("mes_es_exposure_gate_failed")
    if not quote_gate:

"""Separator analysis for the US_MIDDAY SHORT failed_move_reversal lane."""
...
def build_and_write_failed_move_reversal_us_midday_short_separator(*, detail_csv_path: Path) -> dict[str, str]:
    ...
    lane_summary_path = prefix.with_suffix(".pattern_engine_v1_failed_move_reversal_us_midday_short_lane_summary.json")
    candidate_csv_path = prefix.with_suffix(".pattern_engine_v1_failed_move_reversal_us_midday_short_separator_candidates.csv")
    summary_json_path = prefix.with_suffix(".pattern_engine_v1_failed_move_reversal_us_midday_short_separator_summary.json")
...
def _separator_quality(
    *,
    ...
    if (
        ...
        and aligned_rate >= Decimal("0.56")
        and aligned_rate_lift > Decimal("1.03")
...
def _build_summary_payload(*, lane_summary: dict[str, Any], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    broad_noise = [row for row in repeatable_candidates if row["separator_quality"] == "broad_or_noisy"][:8]
    ...
    return {
        ...
        "useful_secondary_discriminators": useful_secondaries,
        "broad_or_noisy_repeatables": broad_noise,
...
def main() -> None:
    ...
    result = build_and_write_failed_move_reversal_us_midday_short_separator(detail_csv_path=args.detail_csv)

...
- Databento continuous symbology for `GLBX.MDP3` must resolve with `stype_in = continuous` and `stype_out = instrument_id` first. The resolved instrument id is the primary Databento quote request symbol.
- Continuous symbol resolution is date-dependent. Track B must resolve with an explicit UTC `resolution_date` or an explicit `resolution_start`/`resolution_end` window, defaulting the read-only quote CLI to today's UTC date when the operator does not provide one.
- Databento mapping intervals must be recorded and the active mapping must satisfy `d0 <= resolution_date < d1`. If no mapping covers the date, quote-derived readiness fails before quote with `no_active_mapping_for_date`.
- Prior-session resolution fallback is forbidden by default. If explicitly enabled for read-only quote diagnostics or paper-only preparation, Track B may search a small bounded prior calendar-day window, must report `requested_resolution_date`, `actual_resolution_date_used`, `prior_session_fallback_used = true`, and must not silently treat the result as current-session/live-money readiness.
- If raw-symbol reporting or comparison is needed, Track B may perform a second optional `instrument_id -> raw_symbol` lookup. If that lookup is unavailable, raw-symbol validation is reported as incomplete rather than silently passed.

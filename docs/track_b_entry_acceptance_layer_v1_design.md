...
- `completed_5m_market_context`: completed 5m OHLCV bars or summarized completed 5m features, including latest completed timestamp, timeframe, directional features, volatility/range features, pullback/retest features, and candle completeness.
...
| Directional alignment | 0.15 | Measures candidate side against completed 5m impulse, close location, trend/reclaim behavior, and directional context. |
...
- Stale/thin data: insufficient completed 5m candles or stale latest timestamp, expected `LOW_CONFIDENCE_INSUFFICIENT_DATA`.
...
- Inputs: completed 5m market context, session calendar labels, instrument metadata, volatility/range summaries, optional historical regime labels, and provenance fields.
